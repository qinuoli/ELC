...
@app.get("/")
def page():
    # 防对话式AI爬取：校验请求头，屏蔽AI/爬虫请求
    user_agent = request.headers.get('User-Agent', '').lower()
    ai_keywords = ['bot', 'crawler', 'spider', 'curl', 'wget', 'python', 'requests', 'scrapy', 'playwright', 'puppeteer', 'chatgpt', 'gpt', 'claude', 'bingbot', 'googlebot', 'baiduSpider']
    if any(keyword in user_agent for keyword in ai_keywords) or re.match(r'^python-requests/\d+\.\d+', user_agent):
        return """<html><head><meta charset="utf-8"></head><body style="text-align:center;margin-top:100px;font-size:20px;">禁止爬虫/AI访问！</body></html>""", 403
...
<html>
  <head>
    ...
    <style>
      ...
      .article-card {{user-select: none; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none;}}
    </style>
    <script>
      // 防AI爬取：禁止右键/查看源码/复制
      ...
      // 防AI动态解析：JS解密逻辑延迟执行+随机干扰
      let decryptDelay = Math.floor(Math.random() * 1000) + 500;
      setTimeout(() => {{
        window.onload = decryptAllArticles;
      }}, decryptDelay);
      // Canvas绘制文字核心函数
      function renderTextToCanvas(container, text) {{
  ...
  <body>
    <img id="steg-icon" src="{base_img_b64}" alt="基础载体图标">
      ...
      async function decryptAllArticles() {{
        ...
        for (const card of articleCards) {{
          try {{
            ...
            // 清空加载占位符，改用Canvas绘制文字
            card.innerHTML = '<div class="article-title">' + card.querySelector('.article-title').textContent + '</div>';
            renderTextToCanvas(card, plaintext);
          }} catch (e) {{
